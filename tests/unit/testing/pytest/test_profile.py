...
# the top-level code in pkglib.testing.pytest.profile will be omitted from
# coverage, so force it to be reloaded within this test unit under coverage
import pkglib.testing.pytest.profile
reload(pkglib.testing.pytest.profile)
from pkglib.testing.pytest.profile import Profiling, pytest_addoption, pytest_configure
...
def test_hooks_pyfunc_call():
    ...
    with patch('os.path.join', return_value=sentinel.join):
        with patch('pkglib.testing.pytest.profile.cProfile') as cProfile:
...
def test_configures():
    ...
    with patch('pkglib.testing.pytest.profile.Profiling') as Profiling:
