...
class Installer(easy_install.Installer):
    ...
    def install(self, specs, working_set=None, use_existing=False, draw_graph=False):
        ...
        # This is the list of stuff we've installed, to hand off to the postinstall
        # steps like egg-link etc
        ...
        # First we need to get a map of requirements for what is currently installed.
        # This is so we can play them off against new requirements.

        # For simplicity's sake, we merge all requirements matching installed packages
        # into a single requirement. This also mimics how the packages would have
        # been installed in the first place.

        # This is a mapping of key -> (dist, originating req) which is our best found so far.
        ...
        requirements.reverse()  # Set up the stack, so we're popping from the front
        ...
        def purge_req(req):
            ...
            if req.key in best:
                del best[req.key]
            [dependency.remove_from_ws(w, req._chosen_dist) for w in (ws, setup_dists)
             if req._chosen_dist in w]
        ...
        while requirements:
            ...
            if prev_req and prev_req.hashCmp != req.hashCmp:
                log.debug("--- checking previously requirements: %s vs %s" % (prev_req, req))
                ...
                # specivicity - ie, creating a new requirement that is bounded
                ...
                try:
                    ...
                    if dist in merged_req:
                        ...
                        # just update the 'best' index to the new one
                        if prev_req.hashCmp != merged_req.hashCmp:
                            log.debug("--- upgrading to more specific requirement %s -> %s" % (prev_req, merged_req))
                            ...
                            if upstream:
                                log.debug("---- adding edges from %s to %s" % (req, upstream))
                                [req_graph.add_edge(req, i) for i in upstream]
                        else:
                            log.debug("--- skipping %s, it's more general than %s" % (req, prev_req))
                            processed[req] = True
                            continue
                            # TODO: look @ req.extras?
                    else:
                        # The more specific version is different to what we've already
                        # found, we need to override it.
                        log.debug("**** overriding requirement %r with %r" % (prev_req, req))

                        # Now we need to purge the old package and everything it brought in, so that
                        # there's no chance of conflicts with the new version we're about to install

                        log.debug("****   resolving possible backtrack targets")

                        upstream_reqs = dependency.get_all_upstream(req_graph, prev_req)
...
def uninstall_eggs(reqs):
    ...
    for name in names:
        ...
        if not dist:
            raise DistutilsOptionError('Cannot remove package not yet installed: %s' % name)
        dist = dist[0]
        if not dist.location.endswith('.egg'):
            raise DistutilsOptionError('Not an egg at %s, chickening out' % dist.location)
        if is_local(dist.location):
            dists.append(dist)
        else:
            log.info("Not uninstalling egg, it's not in our virtualenv: %s" % dist.location)
...
@contextmanager
def prefer_dev():
    """ Patch `zc.buildout.easy_install._final_version` to swap the behavior
        to prefer dev versions instead.
        TODO: find a way to make it work only on in-house packages, so we can
              allow 'dev' version of 3rd party stuff on the PyPI server.
    """
    def dev_version(parsed_version):
        for part in parsed_version:
            if (part[:1] == '*') and (part not in easy_install._final_parts):
                return True
        return False

    orig = easy_install._final_version
    easy_install._final_version = dev_version
    yield
    easy_install._final_version = orig


def install(cmd, reqs, add_to_global=False, prefer_final=True, force_upgrade=False, use_existing=False):
    """ Installs a given requirement using buildout's modified easy_install.
        ...
        add_to_global : `bool`
           adds installed distribution to the global
           working_set. This has the effect of making
           them available for import within this process,
           used by fetch_build_eggs.
        prefer_final : `bool`
           Will prefer released versions of the requirements over
           dev versions
        force_upgrade : `bool`
           Will force downloads of requirements from PyPI. This is the equivalent
           of ``easy_install -U my_org.foo``
        use_existing : `bool`
    ...
    # Create installer class configured to install into wherever the command class
    # was setup for
    ...
    # This is a bit nasty - we have to monkey-patch the filter for final versions
    # so that we can also filter for dev versions as well.
    ...
    if use_existing:
        ...
        #      packages over ones on the server, eg an installed release version
        #      won't get trumped by a dev version on the server
        ...
        # This way we won't get any weird requirement conflicts with new versions
        # of the package we're trying to set up
    ...
    # There's a chance that there were packages in setup_requires that were also
    # in install_requires. Because these are classed as 'already installed' by
    # the installer, they won't have been added to the workingset of packages
    ...
    setup_dists = [i for i in
                   pkg_resources.working_set.resolve(get_setup_requires(cmd.distribution))
                   if also_required(i)]
    ...
    # Now patch the filter method if required and run the installer
    if prefer_final:
    ...
    if to_setup:
        ...
        # Luckily, the buildout installer didnt strip off any of the useful metadata
        # about the console scripts.
        for dist in to_setup:
            ...
            # Add the distributions to the global registry if we asked for it. This makes
            # the distro importable, and classed as 'already installed' by the dependency
...
def get_setup_requires(dist):
    ...
    if reqs:
        return pkg_resources.parse_requirements([i.strip() for i in reqs[1].split('\n')
                                                 if i.strip()])
    return []
