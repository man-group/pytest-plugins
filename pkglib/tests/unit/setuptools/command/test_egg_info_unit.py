...
def test_write_revision():
    ...
    args, _ = cmd.write_file.call_args_list[0]
...
def test_get_new_build_number_not_dev_version():
    cmd = get_cmd('1.0.0')
    cmd.pypi_client.get_last_version.return_value = '1.0.0'
    cmd.new_build = True
...
def test_get_new_build_number_old_style_dev_version():
    cmd = get_cmd('1.0.0')
    cmd.pypi_client.get_last_version.return_value = '1.0.0.dev2'
    cmd.new_build = True
    with pytest.raises(ValueError):
        cmd.finalize_options()
...
def test_get_new_build_number_no_dev_versions_yet():
    cmd = get_cmd('1.0.0')
    cmd.pypi_client.get_last_version.return_value = None
    cmd.new_build = True
    cmd.finalize_options()
...
def test_get_new_build_number():
    cmd = get_cmd('1.0.0')
    cmd.pypi_client.get_last_version.return_value = '0.0.dev2'
    cmd.new_build = True
    cmd.finalize_options()
