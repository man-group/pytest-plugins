...
from .base import fetch_build_eggs
from .test import test


class ext_gcov_test(test):
    """
    Run tests (as per the 'test' command) with gcov on Python extensions to
    generate Cobertura gcov/coverage.xml.
    This command depends on '_build' and 'build_ext_static_interpreter'
    commands; the latter is because gcov doesn't work on dynamically loaded
    modules, so we have to statically link the extensions as builtin modules.
    """
    description = "Run test coverage for Python extensions"
    user_options = test.user_options + [
        ("gcov-base", None,
         "Base directory for gcov output, defaults to %s/gcov" % os.getcwd()),
        ("gcov-coverage-file", None,
         "coverage.xml file, default is {gcov-base}/coverage.xml"),
    ]

    def initialize_options(self):
        self.gcov_base = os.path.join(os.getcwd(), 'gcov')
        self.gcov_coverage_file = None
        super(ext_gcov_test, self).initialize_options()

    def finalize_options(self):
        super(ext_gcov_test, self).finalize_options()
        if not self.gcov_coverage_file:
            self.gcov_coverage_file = os.path.join(self.gcov_base,
                                                   "coverage.xml")

    def run(self):
        gcov = fetch_build_eggs(["gcovr"], dist=self.distribution)[0]
        gcov_script = os.path.join(gcov.egg_info, "scripts/gcovr")
        self.execute(self.clean_gcov_base, (), "Cleaning gcov directory")
        interpreter, uses_cython = self.build_gcov_interpreter()
        log.info("built interpreter: {0}".format(interpreter))

        self.execute(self.fetch_requirements, [], "Fetching test requirements")
        pytest_args, doctest_args = self.get_args()
        if self.doctest:
            self.execute(self.run_gcov_pytest, (doctest_args, interpreter),
                         "Running doctests")
        if self.all or self.unit or self.integration:
            self.execute(self.run_gcov_pytest, (pytest_args, interpreter),
                         "Running tests")

        # Cython emits #line references to 'cython_utility' that confuse gcovr
        if uses_cython:
            log.info("Creating dummy cython_utility for gcovr")
            open(os.path.join(os.getcwd(), 'cython_utility'), 'w').close()

        import re
        gcovr_args = [sys.executable, gcov_script,
                      self.gcov_base, "--root=.",
                      "--exclude=" + re.escape(self.gcov_base) + "/.*",
                      "--exclude=cython_utility",
                      "--exclude=.*\.egg/.*"]
        self.spawn(gcovr_args)  # tabular to stdout
        self.spawn(gcovr_args + ["--xml",
                                 "--output=" + self.gcov_coverage_file])
    ...
    def build_gcov_interpreter(self):
        ...
        esi.interpreter_filename = os.path.join(self.gcov_base,
                                                "gcov_interpreter")
