...
class pyinstall(Command, CommandMixin):
    ...
    user_options = [
        ...
        ('update', 'U',
         "Forces an update of packages and their dependencies, respecting "
         "source checkouts."),
        ...
        ('everything', 'E',
         'Update only: include all currently installed packages'),
    ]
    boolean_options = [
        'dev',
        'update',
    ...
    def finalize_options(self):
        ...
        if not self.update:
            ...
            if self.distribution.get_name() == 'UNKNOWN' and not self.args:
                raise DistutilsOptionError('Please specify some package '
                                           'requirements.')

        if not self.update:
            # If we're not in update mode, figure out whether or not what we've
            # asked to install is already installed, and upgrade it instead -
            # but only if it's a  dev package.
            ...
            installed_req_dists = [i for i in pkg_resources.working_set
                                   if i.project_name in req_names]

            if installed_req_dists:
                # Some of our requirements are already installed. 
                # We swap to update mode but only if they're all being updated
                # at once
    ...
    def run(self):
        ...
        if not self.update:
            install(self.get_easy_install_cmd(index_url=self.index_url),
                    self.args, prefer_final=not self.dev,
                    force_upgrade=self.update)
        else:
            # Update mode - call the pyupdate command
            cls = self.distribution.get_command_class('update')
            cmd = cls(self.distribution, dev=self.dev,
                      everything=self.everything, third_party=self.third_party,
                      source=self.source, eggs=self.eggs,
